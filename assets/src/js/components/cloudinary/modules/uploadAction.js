...
import imgQuantityCheck from './imgQuantityCheck'
import legalesQuantityCheck from './legalesQuantityCheck'
...
var uploaders = {
  ...
  cartCertificatUploader: (e, cb, err) => {
    // cloudinary upload options
    cloudinaryKey.folder = 'certificats_documents'
    cloudinaryKey.field_name = 'photo[]'
    cloudinaryKey.max_files = 1
    ...
    cloudinary.openUploadWidget(cloudinaryKey, (err, res) => {
      ...
      if (res.length >= 1) {
        var certificat = res[0].secure_url
        cb(certificat)
      } else {
        err(certificat.secure_url)
      }
    })
  },
  imageUploader: (e) => {
